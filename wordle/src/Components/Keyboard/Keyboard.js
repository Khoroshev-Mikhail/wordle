...
function Keyboard(props){
    ...
    function keyClass(key){
        return classNames({
            'missedKey': props.attempts.join('').includes(key) && !props.trueLetters.includes(key) &&  !props.includedLetters.includes(key),
            'yellowKey': props.includedLetters.includes(key),
            ...
            <div className="keyboard__row">
                ...
                {firstRowKeyboard.map(key => {
                    return (
                        ...
                        <button className={keyClass(key)} key={key} disabled={props.isWinner} onClick={pressKey} value={key}>{key}</button>
                    )
                })}
            </div>
            <div className="keyboard__row">
                <div className="spacer half"></div>
                {secondRowKeyboard.map(key => <button className={keyClass(key)} key={key} disabled={props.isWinner} onClick={pressKey} value={key}>{key}</button> )}
                <div className="spacer half"></div>
            </div>
            <div className="keyboard__row">
                <button disabled={props.isWinner} onClick={props.tryIt} value="Enter" className="one-and-half">Enter</button>
                {thirdRowKeyboard.map(key => <button className={keyClass(key)} key={key} disabled={props.isWinner} onClick={pressKey} value={key}>{key}</button>)}
